...
const _input = UpdateContainerInput(
  id: 1,
  styleConfig: StyleConfig(marginTop: 4),
);

void main() {
  group('UpdateContainerInTemplateUseCase — admin', () {
    test('forwards input to the repository', () async {
      ...
      final repo = FakeContainerRepository()
        ..whenUpdate(const Success(_container));
      final useCase = UpdateContainerInTemplateUseCase(
        authGateway: gateway,
      ...
      expect(repo.calls.single, isA<ContainerUpdateCall>());
    });
  ...
  group('UpdateContainerInTemplateUseCase — non-admin', () {
