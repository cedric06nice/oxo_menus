...
void main() {
  group('CreateWidgetInTemplateUseCase — admin', () {
    test('forwards input to the repository', () async {
      final gateway = await gatewayFor(adminUser);
      addTearDown(gateway.dispose);
      final repo = FakeWidgetRepository()..whenCreate(const Success(_widget));
      final useCase = CreateWidgetInTemplateUseCase(
        authGateway: gateway,
      ...
      expect(repo.calls.single, isA<WidgetCreateCall>());
    });
  ...
  group('CreateWidgetInTemplateUseCase — non-admin', () {
