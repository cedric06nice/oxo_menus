...
void main() {
  group('ListAvailableMenusForBundlesUseCase — admin', () {
    test('lists every menu (drafts + published) without filtering', () async {
      final gateway = await _gatewayFor(_admin);
      addTearDown(gateway.dispose);
      final repo = FakeMenuRepository()
        ..whenListAll(const Success([_draftMenu, _publishedMenu]));
      ...
      expect(result.valueOrNull, [_draftMenu, _publishedMenu]);
      expect(repo.calls.single, isA<MenuListAllCall>());
      expect((repo.calls.single as MenuListAllCall).onlyPublished, isFalse);
    });
  ...
  group('ListAvailableMenusForBundlesUseCase — non-admin', () {
